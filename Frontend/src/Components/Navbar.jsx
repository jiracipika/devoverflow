...
const Navbar = () => {
  ...
  return (
    <nav className='dark:bg-gray-200 text-white dark:text-black py-10 px-4 min-h-screen flex flex-col justify-between z-10 fixed top-0 max-h-full w-[266px] bg-[#0B0C14]'>
      <div className='flex flex-col w-full gap-8 '>
        <h1 className='px-4 py-2.5 text-[18px]'>Logo</h1>
        <ul className='flex flex-col gap-4 w-full '>
          <NavLink to={"/"} className={({ isActive }) => `flex transition-all text-[18px] px-4 py-2.5 rounded-lg w-full font-normal gap-2 items-center ${isActive ? "bg-custom-gradient" : ""}`}><img className="" src={homeSvgSrc} alt="Home-icon" />Home</NavLink>
